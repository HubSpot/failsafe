...
public class CancellableFuture<T> extends java.util.concurrent.CompletableFuture<T> {
  final FailsafeFutureOld<T> future;

  private CancellableFuture(FailsafeFutureOld<T> future) {
    this.future = future;
  }
   ...
   */
  public static <T> java.util.concurrent.CompletableFuture<T> of(FailsafeFutureOld<T> future) {
