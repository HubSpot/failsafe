...
public class FailsafeFutureOld<T> implements Future<T> {
  ...
  FailsafeFutureOld(FailsafeConfig<T, ?> config) {
    this.config = config;
    circuit.open();
  }
  ...
  void inject(ExecutionContextOld execution) {
