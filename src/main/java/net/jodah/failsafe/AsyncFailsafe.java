...
import net.jodah.failsafe.function.AsyncRunnableOld;
...
/**
 * Performs asynchronous executions withMigration failures handled according to a configured {@link #with(RetryPolicy) retry
...
public class AsyncFailsafe<R> extends AsyncFailsafeConfig<R, AsyncFailsafe<R>> {
   ...
   * <p>
   * If a configured circuit breaker is open, the resulting future is completed exceptionally withMigration
  ...
  /**
   * Executes the {@code callable} asynchronously until the resulting future is successfully completed or the configured
   * {@link RetryPolicy} is exceeded. This method is intended for integration withMigration asynchronous code. Retries must be
   ...
   * {@link RetryPolicy} is exceeded.
   * <p>
   * If a configured circuit breaker is open, the resulting future is completed withMigration
   * {@link CircuitBreakerOpenException}.
   *
   * @throws NullPointerException if the {@code callable} is null
   */
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public <T> FailsafeFutureOld<T> get(Callable<T> callable) {
  ...
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public <T> FailsafeFutureOld<T> get(ContextualCallable<T> callable) {
    return call(Functions.asyncOf(callable), null);
  }
  ...
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public <T> FailsafeFutureOld<T> getAsync(AsyncCallable<T> callable) {
    return call(Functions.asyncOf(callable), null);
  }
  ...
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public FailsafeFutureOld<Void> run(CheckedRunnable runnable) {
  ...
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public FailsafeFutureOld<Void> run(ContextualRunnableOld runnable) {
    return call(Functions.<Void>asyncOf(runnable), null);
  }
  ...
  // Migrate usage of `Failsafe.with` to convert from 1.x `SyncFailsafe` to 2.x `FailsafeExecutor`
  @Deprecated
  public FailsafeFutureOld<Void> runAsync(AsyncRunnableOld runnable) {
    return call(Functions.<Void>asyncOf(runnable), null);
  }
  ...
  private <T> java.util.concurrent.CompletableFuture<T> call(AsyncCallableWrapper<T> callable) {
    FailsafeFutureOld<T> future = new FailsafeFutureOld<T>((FailsafeConfig<T, ?>) this);
    java.util.concurrent.CompletableFuture<T> response = net.jodah.failsafe.CancellableFuture.of(future);
   ...
   */
  @SuppressWarnings("unchecked")
  private <T> FailsafeFutureOld<T> call(AsyncCallableWrapper<T> callable, FailsafeFutureOld<T> future) {
    if (future == null)
      future = new FailsafeFutureOld<T>((FailsafeConfig<T, ?>) this);
    ...
    AsyncExecutionOld execution = new AsyncExecutionOld(callable, scheduler, future, (FailsafeConfig<Object, ?>) this);
